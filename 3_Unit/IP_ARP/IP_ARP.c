/*Notes: Windows doesn't seem to recognize T[18] as number 6 for example in the ARP protocol, so I needed to use the number instead
of the variable, this could lead to several errors*/
...
int analizaTrama(BYTE t[], BYTE resultado[])
{
	...
	else if(ToT == 2048) //Tipo IP
	{
		...
		{
			sprintf(resultado + strlen(resultado), "\nOpciones: ");
			for(int i = 34; i < 34 + (((t[14] & 15) * 4) - 20); i++)
			{
				if(i == 33 + (((t[14] & 15) * 4) - 20))
	...
	else if(ToT == 2054) //ARP
	{
		...
		{
			if(i == 3)
				sprintf(resultado + strlen(resultado), "%d", t[22 + i + 6]);	
			else
				sprintf(resultado + strlen(resultado), "%d.", t[22 + i + 6]);	
		...
		{
			if(i == 5)
				sprintf(resultado + strlen(resultado), "%.2x", t[22 + i + 6 + 4]);
			else
				sprintf(resultado + strlen(resultado), "%.2x:", t[22 + i + 6 + 4]);
		}
		...
		{
			if(i == 3)
				sprintf(resultado + strlen(resultado), "%d", t[22 + i + 6 + 4 + 6]);
			else
				sprintf(resultado + strlen(resultado), "%d.", t[22 + i + 6 + 4 + 6]);
		}

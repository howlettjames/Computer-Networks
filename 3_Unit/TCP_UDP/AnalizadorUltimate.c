...
int analizaTrama(BYTE t[], BYTE resultado[])
{
	...
	else if(ToT == 2048) //Tipo IP
	{
		...
		{
			sprintf(resultado + strlen(resultado), "\n----------ICMP----------\n");	
			sprintf(resultado + strlen(resultado), "Type: %s\n", icmp_type[t[14 + ihl] & 31]); //It is 14 beacuse of Ethernet Header
			//if(t[34 + (ihl - 20)] == 3) Es lo mismo al parecer
			if((t[14 + ihl] & 31) == 3)
		...
		else if(t[23] & 6) //TCP
		{
			...
			sprintf(resultado + strlen(resultado), "Sequence Number: %d\n", (t[18 + ihl] << 24) + (t[19 + ihl] << 16) + (t[20 + ihl] << 8) + t[21 + ihl]);	
			sprintf(resultado + strlen(resultado), "Acknowledgement Number: %d\n", (t[22 + ihl] << 24) + (t[23 + ihl] << 16) + (t[24 + ihl] << 8) + t[25 + ihl]);
			unsigned short data_offset = ((t[26 + ihl] >> 4) & 15) * 4;
			sprintf(resultado + strlen(resultado), "Data Offset: %d bytes\n", data_offset);
			sprintf(resultado + strlen(resultado), "Reserved: %d\n", t[26 + ihl] & 15);
			if(t[27 + ihl] & 128)
				sprintf(resultado + strlen(resultado), "CWR On\n");
			else
				sprintf(resultado + strlen(resultado), "CWR Off\n");

			if(t[27 + ihl] & 64)
				sprintf(resultado + strlen(resultado), "ECN-Echo On\n");
			else
				sprintf(resultado + strlen(resultado), "ECN-Echo Off\n");

			if(t[27 + ihl] & 32)
				sprintf(resultado + strlen(resultado), "U On\n");
			else
				sprintf(resultado + strlen(resultado), "U Off\n");

			if(t[27 + ihl] & 16)
				sprintf(resultado + strlen(resultado), "A On\n");
			else
				sprintf(resultado + strlen(resultado), "A Off\n");

			if(t[27 + ihl] & 8)
				sprintf(resultado + strlen(resultado), "P On\n");
			else
				sprintf(resultado + strlen(resultado), "P Off\n");

			if(t[27 + ihl] & 4)
				sprintf(resultado + strlen(resultado), "R On\n");
			else
				sprintf(resultado + strlen(resultado), "R Off\n");

			if(t[27 + ihl] & 2)
				sprintf(resultado + strlen(resultado), "S On\n");
			else
				sprintf(resultado + strlen(resultado), "S Off\n");

			if(t[27 + ihl] & 1)
				sprintf(resultado + strlen(resultado), "F On\n");
			else
				sprintf(resultado + strlen(resultado), "F Off\n");

			sprintf(resultado + strlen(resultado), "Window: %d\n", (t[28 + ihl] << 8) + t[29 + ihl]);	
			sprintf(resultado + strlen(resultado), "Checksum: %.4X\n", (t[30 + ihl] << 8) + t[31 + ihl]);		
			if(t[27 + ihl] & 32) //If URG is on
				sprintf(resultado + strlen(resultado), "Urgent Pointer: %d", (t[32 + ihl] << 8) + t[33 + ihl]);	
			else
				sprintf(resultado + strlen(resultado), "No Urgent Pointer");

			if(data_offset == 20) //No options
				sprintf(resultado + strlen(resultado), "\nNo Options");
			else
			{
				sprintf(resultado + strlen(resultado), "\nOptions: ");
				for(BYTE i = 34 + ihl; i < (34 + ihl) + (data_offset - 20); i++)
				{
					if(i == (34 + ihl) + (data_offset - 20) - 1)
						sprintf(resultado + strlen(resultado), "%.2X", t[i]);	
					else
						sprintf(resultado + strlen(resultado), "%.2X:", t[i]);	
				}
